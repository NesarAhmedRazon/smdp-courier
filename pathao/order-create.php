...
function pathaw_create_new_delivery($order_id,$old_status){
    ...
    $package_weight = get_post_meta($order_id, '_pkg_weight', true) ?: get_post_meta($order_id, 'pkg_weight', true);
    ...
    error_log('Pathao Payload: '.$payload);
// API Endpoint
...
function pathaw_order_status_changed($order_id, $old_status, $new_status, $order) {
    ...
    if ('ready-to-shipping' === $new_status && 'order-processing' === $old_status) {  
        ...
        switch ($provider) {
            case 'pathaw':
                error_log('Pathao Order Create Triggered for Order ID: '.$order_id);

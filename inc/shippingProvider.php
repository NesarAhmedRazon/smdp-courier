...
function smdp_get_shipping_providers()
{
    ...
    return [
        ''         => __('Select a Provider', SMDP_TEXTDOMAIN),
        'pathaw'   => 'পাঠাও',
        'redx'     => 'রেডএক্স',
        'ecourier' => 'ই-কুরিয়ার',
        'paperfly' => 'পেপারফ্লাই',
...
function smdp_add_provider_to_method($fields)
{
    $fields['shipping_provider'] = [
        'title'       => __('Shipping Provider', SMDP_TEXTDOMAIN),
        'type'        => 'select',
        'class'       => 'wc-enhanced-select',
        'description' => __('Select the shipping provider for this method.', SMDP_TEXTDOMAIN),
        'desc_tip'    => true,
        'default'     => '',
...
if(!function_exists('smdp_get_order_shipping_provider')){
    // Get the shipping provider for a specific order
    function smdp_get_order_shipping_provider($order)
    {
...
if(!function_exists('smdp_get_shipping_state_label')){
